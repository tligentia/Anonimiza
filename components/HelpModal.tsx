...
const HelpModal: React.FC<HelpModalProps> = ({ isOpen, onClose }) => {
  ...
  const steps = [
    ...
    },
    {
      num: "02",
      title: "Configuración",
      desc: "Añade reglas manuales en el 'Inventario' si necesitas forzar alias específicos o ignorar términos detectados por el motor."
    },
    {
      num: "03",
      title: "Procesamiento",
    ...
    <div className="fixed inset-0 z-[130] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white border-2 border-black max-w-4xl w-full shadow-[20px_20px_0px_rgba(0,0,0,0.1)] rounded-sm overflow-hidden">
        <div className="bg-black text-white p-6 flex justify-between items-center">
          <h2 className="text-sm font-black uppercase tracking-[0.3em]">Guía de Operación ANON_CORE</h2>
        ...
        <div className="p-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {steps.map((s, i) => (
            <div key={i} className="space-y-4">
              <div className="text-4xl font-black text-gray-100 group-hover:text-red-600 transition-colors border-b-2 border-gray-50 pb-2">{s.num}</div>
        ...
        <div className="bg-gray-50 p-8 border-t border-gray-200">
          ...
          <div className="mt-8 flex justify-center">
            <button 
              onClick={onClose}
              className="bg-black text-white px-12 py-3 text-[10px] font-black uppercase tracking-widest hover:bg-red-600 transition-all"
            >
              Comenzar a Trabajar
            </button>
          </div>
